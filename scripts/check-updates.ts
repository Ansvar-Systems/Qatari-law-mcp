#!/usr/bin/env tsx
/**
 * Qatari Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.almeezan.qa';
const PORTAL_NAME = 'Al Meezan (Qatar Legal Portal)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/qatari-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Qatari Law MCP — Data Freshness Check');
