/**
 * Tool registry for Qatari Law MCP Server.
...
import { getQatariImplementations, type GetQatariImplementationsInput } from './get-qatari-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Official Gazette of the State of Qatar. ' +
    ...
    'Call this FIRST when you need to understand what Qatari legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Qatari statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from a Qatari statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate a Qatari legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Qatari statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Qatari?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Qatari legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Qatari statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get international legal instruments that a Qatari statute references or aligns with. ' +
      'Qatari laws may reference international standards and conventions ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Qatari statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_qatari_implementations',
    description:
      'Find all Qatari statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Qatari statutes. ' +
      'Qatari may reference international legal instruments where relevant.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Qatari legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_qatari_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Qatari legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within a Qatari statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for a Qatari statute or provision. ' +
      ...
      'Qatari is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Qatari statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_qatari_implementations':
          result = await getQatariImplementations(db, args as unknown as GetQatariImplementationsInput);
          break;
