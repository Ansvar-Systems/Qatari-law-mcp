/**
 * search_eu_implementations — Search EU directives/regulations referenced by Qatari legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_qatari_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as qatari_statute_count
  ...
  if (input.has_qatari_implementation) {
    sql += ' HAVING qatari_statute_count > 0';
  }
