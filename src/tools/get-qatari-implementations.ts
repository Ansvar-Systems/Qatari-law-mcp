/**
 * get_qatari_implementations — Find Qatari statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetQatariImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface QatariImplementationResult {
  document_id: string;
...
export async function getQatariImplementations(
  db: InstanceType<typeof Database>,
  input: GetQatariImplementationsInput,
): Promise<ToolResponse<QatariImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as QatariImplementationResult[];
